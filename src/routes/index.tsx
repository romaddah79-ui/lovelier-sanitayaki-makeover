    ...
    <>
        ...
        <div className="grid overflow-hidden rounded-lg border border-border bg-card/80 shadow-xs backdrop-blur-xl lg:grid-cols-2">
          <div className="flex flex-col justify-center p-6 sm:p-10">
            ...
            <div className="mt-6 space-y-3">
              {["Bee Venom Cream", "Scorpion Cream"].map((name) => <div key={name} className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 rounded-md border border-border bg-background p-3"><div className="min-w-0"><p className="truncate text-sm font-bold">{name}</p><p className="text-xs text-muted-foreground">Detalii și disponibilitate în pagina produsului</p></div><Button size="sm" variant="outline">Vezi <ChevronRight /></Button></div>)}
            </div>
          </div>
      ...
      <section className="market-section">
        <div className="relative overflow-hidden rounded-lg bg-foreground p-7 text-background sm:p-10">
          ...
          <div className="relative max-w-xl"><span className="inline-flex items-center gap-2 text-xs font-extrabold text-sale"><Sparkles className="size-4" /> OFERTE SPECIALE</span><h2 className="mt-3 font-display text-3xl font-bold">Produsele potrivite, mai ușor de găsit.</h2><p className="mt-3 text-sm leading-6 text-background/70">Consultă selecțiile și prețurile actuale direct în catalogul Sanitayaki.</p><Button asChild className="mt-6"><Link to="/magazin">Vezi ofertele <ArrowRight /></Link></Button></div>
        </div>
      </section>
